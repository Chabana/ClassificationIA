...
class FileSelector:
    ...
    def __init__(self, tabPos, tabNeg):
        """
        Constructeur
        :param tabPos: tableau des fichier test positig
        :param tabNeg: tableau des fichier test negatif
        :return:
        """
    ...
    def isTestPos(self, name):
        """
        Fichier est-il un test positif
        :param name: fichier
        :return:
        """
    ...
    def isTestNeg(self, name):
        """
        Fichier est-il un test negatif
        :param name: fichier
        :return:
        """
    ...
    def isTrainPos(self, name):
        """
        Fichier est-il un entrainement positif
        :param name: fichier
        :return:
        """
    ...
    def isTrainNeg(self, name):
        """
        Fichier est-il un entrainement negatif
        :param name: fichier
        :return:
        """
    ...
    def isInTab(self, name, tab):
        """
        si le fichier est dans le tableua
        :param name: fichier
        :param tab: tableau
        :return:
        """
...
def numToText(n):
    """
    numero du fichier en string
    :param n: numero du fichier
    :return: string du numero du fichier
    """
    s = str(n) + ".txt"
    while len(s) < 8:
        s = "0" + s
...
def createRandom(nb, prop):
    """
    Cree un selection aleatoire
    :param nb: nombre total
    :param prop: proportion train
    :return:
    """
    toSelect = int(nb * prop)

    selected=[]
    noSelected=list(range(nb))
...
def generateBloc(n):
    ...
    """
    if nbFiles % n != 0:
        raise Exception("can't cut %d files in %d blocs" % (nbFiles, n))
    nbPerBloc = int(nbFiles / n)
    x = [numToText(i) for i in range(nbFiles)]
    random.shuffle(x)
    return [x[i* nbPerBloc:(i+1) * nbPerBloc] for i in range(n)]

def createRandomSelectors(n):
    """
    Cree n corpus aleatoir
    :param b: nombre
    """
    for i in range(n):
...
def createBlockSelectors(n):
    """
    Cree n corpus par bloc de taille 1000/n
    :param b: nombre
    """
...
class FileSelectorDeterminist(FileSelector):
    """
    Corpus déterministe, a chaque lancement les mêmes fichiers
    """
    def __init__(self):
